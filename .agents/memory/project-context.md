...
| State | F11 Forms+Fields merge complete (2026-05-03). Fields no longer have a dedicated route — they are managed inside the form-builder via the `Editar campos` dialog. Forms now carry an array of `FormFieldInstance` (`{ instanceId, fieldId, labelOverride? }`), so the same field can appear more than once with independent labels and values. Observations key their `values` by `instanceId`. Dexie v8 hard reset (forms, observations, chronicles, media). Export/Import bumped to `chronicle-full-v3`; legacy v1/v2/encounter-v1 are rejected. Onboarding tour rewritten (no more Campos hub-stop). |
