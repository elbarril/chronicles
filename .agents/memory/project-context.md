...
| State | F7 Optional Gemini AI chronicle generation completed (2026-05-01) — BYOK API key in `localStorage`, graceful fallback to deterministic, `generatedWith` on `Chronicle`, `/settings` route, `AiSetupGuide` component, 3-step onboarding, "Generada con IA" badge, `infra/ai/` layer, unit/E2E tests green |
...
- **Chronicles:** deterministic generation template from encounter data (group/activity/timestamps + observation title + details), unified list-table view, regenerate/delete actions, inline media gallery in detail. Optional AI generation via Gemini API (BYOK); badge shown when `generatedWith === "gemini"`.
...
- **Onboarding:** first-run welcome dialog (3 steps: what is Chronicle / data storage / AI setup optional) gated by `chronicle.onboardingCompleted` in `localStorage`.
- **Settings:** `/settings` route with Gemini API key management; key stored in `localStorage` under `chronicle.geminiApiKey`; masked input with show/hide toggle and clear action.
...
- **Current testing:** 104 unit tests + E2E tests across all phases; F7 adds `tests/unit/settings-service.test.ts`, `tests/unit/gemini-chronicle-generator.test.ts`, updated `tests/unit/chronicle-service.test.ts`, updated `tests/unit/onboarding-dialog.test.tsx`, `tests/e2e/settings-api-key.spec.ts`, `tests/e2e/chronicle-ai-generation.spec.ts`.
