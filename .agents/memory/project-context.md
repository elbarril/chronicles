...
- **Encounters:** post-event records living inside a project. `name`, `startsAt`, `endsAt`, `participantIds[]` (subset of project participants). Archive/restore plus full edit (since F12) at `/encounters/:id/edit`. The detail header lists the actual attendees as a chip list.
